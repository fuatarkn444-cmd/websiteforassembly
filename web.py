...
# 2. VERİTABANI YÖNETİMİ(JavaScript; verileri saklamak ve aktarmada da kullanılan bu)
# =====================================================================
DB_FILE = "db.json"
...
def load_db():
    default_db = {
        ...
        "completed_jobs": [], "errors": [], "work_order_templates": [] 
    }
...
# ==süreyi hesaplamak ve dönüştürmek için
...
def stop_timers(istasyon_verisi):
    ...
    if istasyon_verisi["status"] in ["Duruş", "Boşta Duruş"] and istasyon_verisi.get("last_break_start"):
    ...
    if aktif_rol == "Yönetici":
        ...
        with tab1:
            ...
            for ist in ["Montaj-1", "Montaj-2", "Montaj-3"]:
                ...
                elif veri["status"] == "Boşta Duruş":
                    durum_gosterim = f"🛑 Duruş ({veri.get('break_reason', '')}) - İşsiz"
                elif veri["status"] == "Duruş":
                ...
                tablo_verisi.append({
                    ...
                    "Anlık Duruş": format_dk_sn(anlik_mola) if anlik_mola > 0 else "-",
            ...
            for index, istasyon in enumerate(["Montaj-1", "Montaj-2", "Montaj-3"]):
                with [btn_c1, btn_c2, btn_c3][index]:
                    if st.button(f"🚨 {istasyon} ANLIK DURDUR", key=f"dur_{istasyon}", use_container_width=True):
        ...
        with tab2:
            ...
            col_ata_sol, col_ata_sag = st.columns([2, 1])
                ...
                with btn_satir_1:
                    if st.button("🚀 İş Emrini Gönder", type="primary", use_container_width=True):
                        ...
                        if is_urgent:
                            ...
                            st.success("Acil İş Emri iletildi!")
                        else:
                            hedef_veri["pending_jobs"].append(yeni_is_paketi)
                            st.success("İş Emri personelin onay havuzuna gönderildi.")
                        save_db(db)
                        time.sleep(1)
                        st.rerun()
                ...
                with btn_satir_2:
                    if st.button("💾 Şablon Olarak Kaydet", use_container_width=True):
                        if wo_id != "" and not any(t["wo_id"] == wo_id for t in db["work_order_templates"]):
                            ...
                            save_db(db)
                            st.success("Şablon başarıyla yan tarafa kaydedildi.")
                            time.sleep(1)
                            st.rerun()
                        else:
                            st.warning("Bu şablon zaten mevcut veya form boş.")

            with col_ata_sag:
                st.markdown("### 📁 Kayıtlı Şablonlar")
                if len(db.get("work_order_templates", [])) == 0:
                    st.info("Henüz kayıtlı bir iş emri şablonu yok.")
                else:
                    for t in reversed(db["work_order_templates"]):
                        st.success(f"**İş Emri:** {t['wo_id']}\n\n**Ürün SN:** {t['sn_id']}")
        ...
        with tab4:
            ...
            if db["completed_jobs"]:
                ...
                for job in reversed(db["completed_jobs"]):
                    ...
                    sicil_data.append({
                        "Tarih": job["date"], "İstasyon": job["station"], "İş Emri": job["id"], "SN": job["sn"],
                        "Tamamlanan": f"{job.get('target_qty', '?')} Adet", "Hata": hata_durumu, "Hata Notu": hata_notu
                    })
        ...
        if ist.get("urgent_alert"):
            st.markdown(f"""
                <div class='urgent-alert'>
                    ...
                    <h1 style='color: yellow;'>İş Emri: {ist['pending_urgent_job']['id']} | Adet: {ist['pending_urgent_job']['target_qty']}</h1>
                </div><br>
            """, unsafe_allow_html=True)
            ...
            with c1:
                if st.button("⚡ HEMEN GEÇ (Mevcut İşi Askıya Al)", type="primary", use_container_width=True):
                    ...
                    st.rerun()
            with c2:
                if st.button("⏳ BEKLET (Onay Kutusuna At)", use_container_width=True):
                    ist["urgent_alert"] = False
        ...
        # --- OPERATÖR BÖLÜM 2: NORMAL İŞLEYİŞ ---
            ...
            if len(ist.get("pending_jobs", [])) > 0:
                with st.container(border=True):
                    ...
                    for idx, pj in enumerate(ist["pending_jobs"]):
                        ...
                        col_p1.write(f"**İş:** {pj['id']} | **Ürün:** {pj['sn']} | **Adet:** {pj['target_qty']}")
            ...
            if len(all_queued) > 0:
                with st.expander(f"📋 KUYRUKTAKİ VE ASKIDAKİ İŞLER ({len(all_queued)} Adet)"):
                    for idx, qj in enumerate(ist.get("job_queue", [])):
                        ...
                        if col_q2.button("Geçiş Yap", key=f"sw_q_{idx}"):
                            if ist["id"] != "":
                                stop_timers(ist)
                                ist["suspended_jobs"].append({"id": ist["id"], "sn": ist["sn"], "target_qty": ist["target_qty"], "current_qty": ist["current_qty"], "step": ist["step"]})
            ...
            if durum not in ["Bekliyor", "Boşta Duruş", "Tamamlandı"]:
                with st.container(border=True):
                    ...
                    with col_i3:
                        ...
                        is_active_b = "true" if durum in ["Duruş", "Boşta Duruş"] else "false"
                        ...
                        components.html(
                            ...
                            <div style="font-family: 'Helvetica Neue', sans-serif; display: flex; gap: 15px; justify-content: center;">
                                ...
                                <div style="text-align: center;">
                                    <div style="font-size: 14px; color: #555; margin-bottom: 5px;">🛑 Duruş</div>
            ...
            if durum == "Bekliyor" or durum == "Boşta Duruş":
                if durum == "Bekliyor":
                    ...
                    if st.button("▶️ DURUŞU BİTİR (Beklemeye Dön)", type="primary", use_container_width=True):
                        stop_timers(ist)
                        ist["status"] = "Bekliyor"
            ...
            elif durum == "Duruş" or durum == "Duraklatıldı":
                mesaj = f"DURUŞTA ({ist['break_reason']})" if durum == "Duruş" else "DURAKLATILDI"
                ...
                if step <= 4:
                        ...
                        if step == 3: 
                            ...
                            if st.button("✔️ ONAYLA VE GEÇ", type="primary", use_container_width=True):
                                if qc_pass == USERS["kalite1"]["pass"]:
                                    if ist.get("qc_req_time"): ist["qc_wait_time"] = ist.get("qc_wait_time", 0.0) + (time.time() - ist["qc_req_time"])
                                    ist["step"] = 4
                            ...
                            if st.button("✅ ADIMI TAMAMLA", type="primary", use_container_width=True):
                                if step == 2: ist["qc_req_time"] = time.time()
                                ist["step"] += 1
                                save_db(db)
                                st.rerun()
                else:
                    ...
                    if st.button("📦 SIRADAKİ PARÇAYI AL", type="primary", use_container_width=True):
                        ...
                        if ist["current_qty"] < ist["target_qty"]:
                            ...
                            ist["id"], ist["sn"], ist["target_qty"], ist["current_qty"], ist["step"] = "", "", 0, 0, 1
                        save_db(db)
                        st.rerun()
                    ...
                    with st.popover("🔄 İŞİ BAŞKA İSTASYONA AKTAR", use_container_width=True):
                        ...
                        if st.button("İşi Aktar", type="primary"):
                            if db["stations"][hedef]["status"] in ["Bekliyor", "Tamamlandı"]:
                                ...
                                db["stations"][hedef]["id"], db["stations"][hedef]["sn"], db["stations"][hedef]["target_qty"] = ist["id"], ist["sn"], ist["target_qty"]
                                db["stations"][hedef]["current_qty"], db["stations"][hedef]["step"] = ist["current_qty"], ist["step"]
            ...
            with st.expander("⚠️ HATA BİLDİR (Aktif veya Geçmiş Montajlar İçin)"):
                ...
                secenekler = []
                if aktif_is_secenegi: secenekler.append(aktif_is_secenegi)
                    ...
                    if st.button("Hatayı Yöneticiye Gönder", type="primary", use_container_width=True):
                        if hata_bolgesi != "Seçilmedi" and hata_aciklama != "":
                            ...
                            db["errors"].append({
                                ...
                                "İstasyon": aktif_rol, "İş Emri": hedef_is_id,
                                "Montaj_Donemi": "Yeni Montaj" if "Aktif" in secilen_is_emri else "Geçmiş Montaj",
                                "Hatali_Adim": hatali_adim, "Onceden_Hatali": onceden_hatali,
                                "Bölge": hata_bolgesi, "Açıklama": hata_aciklama,
                                "Foto_Base64": foto_base64, "is_new": True 
                            })
                            save_db(db)
